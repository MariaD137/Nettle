...
async function owner() {
  ...
  const project = createProject(user.id, "Owned Project");
  return { user, project };
}

test("the owner gets their project back", async () => {
  const { user, project } = await owner();
  const result = getOwnedProject(project.id, user.id);
  assert.ok(result);
...
test("a different user gets null, not the project", async () => {
  ...
  assert.equal(getOwnedProject(project.id, intruder.id), null);
});

test("no userId at all gets null, even for a real project id", async () => {
  const { project } = await owner();
  assert.equal(getOwnedProject(project.id, undefined), null);
  assert.equal(getOwnedProject(project.id, null), null);
});

test("a project id that doesn't exist gets null, not a thrown error", async () => {
  const { user } = await owner();
  assert.equal(getOwnedProject("00000000-0000-0000-0000-000000000000", user.id), null);
});

test("no projectId at all gets null", async () => {
  const { user } = await owner();
  assert.equal(getOwnedProject(undefined, user.id), null);
  assert.equal(getOwnedProject(null, user.id), null);
  assert.equal(getOwnedProject("", user.id), null);
});
...
test("a user's second project doesn't get returned for a different project id they also own — no accidental wildcard match", async () => {
  ...
  const projectB = createProject(user.id, "Second Owned Project");

  const resultForA = getOwnedProject(projectA.id, user.id);
  const resultForB = getOwnedProject(projectB.id, user.id);
