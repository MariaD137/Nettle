...
test("a non-default key's secret is never stored in the api_keys table — only its hash", async () => {
  ...
  const project = createProject(user.id, "Hashing Target");

  const created = createApiKey(project.id, "CI key", ["scan"]);
  const row = db.prepare("SELECT key, key_masked FROM api_keys WHERE id = ?").get(created.id) as { key: string; key_masked: string };
  ...
  assert.deepEqual(resolveApiKey(created.key, "scan"), { projectId: project.id });
});
...
test("the default key row still stores its secret in plain, matching projects.api_key — a documented, deliberate exception", async () => {
  ...
  const project = createProject(user.id, "Default Key Target");

  const defaultRow = getDefaultApiKeyRow(project.id)!;
  assert.equal(defaultRow.key, project.apiKey, "still a plaintext mirror of projects.api_key, by design");

  assert.deepEqual(resolveApiKey(project.apiKey, "scan"), { projectId: project.id });
});
...
test("rotating a non-default key stores the new secret hashed too, and the old value stops working", async () => {
  ...
  const project = createProject(user.id, "Rotate Target");

  const created = createApiKey(project.id, "CI key", ["scan"]);
  const rotated = rotateApiKeyById(created.id)!;

  assert.notEqual(rotated.key, created.key);
  const row = db.prepare("SELECT key FROM api_keys WHERE id = ?").get(created.id) as { key: string };
  assert.equal(row.key, sha256(rotated.key));

  assert.equal(resolveApiKey(created.key), null, "the pre-rotation key must be dead");
  assert.deepEqual(resolveApiKey(rotated.key, "scan"), { projectId: project.id });
});
...
test("rotating the default key keeps storing it in plain", async () => {
  ...
  const project = createProject(user.id, "Rotate Default Target");

  const defaultRow = getDefaultApiKeyRow(project.id)!;
  const rotated = rotateApiKeyById(defaultRow.id)!;

  const row = db.prepare("SELECT key FROM api_keys WHERE id = ?").get(defaultRow.id) as { key: string };
...
test("backfillHashedApiKeys migrates a pre-existing plaintext non-default row to hashed, and it keeps authenticating", async () => {
  ...
  const project = createProject(user.id, "Backfill Target");
  ...
  assert.equal(resolveApiKey(legacyKey)?.projectId, project.id, "a not-yet-backfilled plaintext row must still authenticate");

  backfillHashedApiKeys();

  const row = db.prepare("SELECT key, key_masked FROM api_keys WHERE id = ?").get(id) as { key: string; key_masked: string };
  assert.equal(row.key, sha256(legacyKey));
  assert.equal(row.key_masked, maskKey(legacyKey));
  assert.deepEqual(resolveApiKey(legacyKey), { projectId: project.id }, "must still authenticate after migration to the hashed form");
});
...
test("backfillHashedApiKeys is a no-op for rows that already have key_masked set", async () => {
  ...
  const project = createProject(user.id, "Backfill Noop Target");
  const created = createApiKey(project.id, "Already hashed", ["scan"]);

  const before = db.prepare("SELECT key, key_masked FROM api_keys WHERE id = ?").get(created.id);
  backfillHashedApiKeys();
  const after = db.prepare("SELECT key, key_masked FROM api_keys WHERE id = ?").get(created.id);
