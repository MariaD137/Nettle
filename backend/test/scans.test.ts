...
test("recordScan snapshots the project's environment onto the stored report", async () => {
  ...
  const project = await createProject(user.id, "Env Tag Target", { environment: "staging" });
...
test("a scan's environment tag is a permanent snapshot — relabeling the project later doesn't change it", async () => {
  ...
  const project = await createProject(user.id, "Relabeled Project", { environment: "development" });

  const stored = await recordScan(project.id, runScan(CLEAN_APP));
  assert.equal(stored.report.environment, "development");

  await updateProject(project.id, { environment: "production" });

  const reloaded = await getLatestScan(project.id);
...
test("recordScan persists scannerVersion and semgrepVersion as first-class, queryable fields", async () => {
  ...
  const project = (await createProject(user.id, "Version Tracking Target")).id;

  const report = runScan(FLAWED_APP);
  assert.ok(report.scannerVersion, "a real scan should always carry the scanner's own version");

  const stored = await recordScan(project, report);
  ...
  const reloaded = await getLatestScan(project);
...
test("a scan recorded with no semgrepVersion (e.g. a URL scan) stores null, not undefined or a crash", async () => {
  ...
  const project = (await createProject(user.id, "No Semgrep Version Target")).id;
  ...
  assert.equal((await getLatestScan(project))?.semgrepVersion, null);
});
