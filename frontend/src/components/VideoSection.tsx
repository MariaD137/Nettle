...
// JS bundle and can be swapped without a rebuild. Unlike the earlier cut
// this replaces, this one has no captions burned into the picture, so the
// WebVTT <track> below actually renders live captions rather than
// duplicating text that's already in the frame.
 ...
 * means the browser fetches nothing but the (lightweight) poster until the
 * visitor actually presses play, so the video's own weight never counts
 * against the landing page's load time. The poster's fixed aspect-ratio on
 * the wrapping frame reserves the video's layout space before either asset
 * loads, so there's no layout shift either way.
 */
